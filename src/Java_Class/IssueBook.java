...
public class IssueBook extends javax.swing.JFrame {
   ...
   int bookcount=0;
    ...
    public boolean getStudentDetails() {
        ...
        try {
            Connection con = ConnectionProvider.getCon();
            String sql1 = "SELECT COUNT(*) FROM issue_book_details WHERE student_id = ? and status =?";
            ...
            if (rs.next()) {
                int issuedBookCount = rs.getInt(1);
                bookcount=issuedBookCount;
                //System.out.println(student_id + " has issued " + issuedBookCount + " books.");
            } else {
    ...
    // method to count number of books that a student issued
    public int CountNoOfBook() {
        int isCount = 0;
        String student_id = txt_student_id.getText();

        try {
            Connection con = ConnectionProvider.getCon();
            String sql1 = "SELECT COUNT(*) FROM issue_book_details WHERE student_id = ?";
            PreparedStatement ps = con.prepareStatement(sql1);
            ps.setString(1, student_id);
        ...
        if (txt_quantity.getText().equals("0")) {
            ...
            if (isAlreadyIssued() == false) {
                ...
                if ((getBookDetails() == false) || (getStudentDetails() == false)) {
                    ...
                    if (bookcount == 3) {
                        JOptionPane.showMessageDialog(this, "Student Has Maximum Number Of Books!");
