...
public class RenewBook extends javax.swing.JFrame {
    ...
    public boolean RenewBook() {
        ...
        try {
            Connection con = ConnectionProvider.getCon();
            String sql = "Update issue_book_details set issue_date=? ,due_date = ?, fine_days=?, total_fine=? where student_id =? and book_id =? and status =?";
            PreparedStatement ps = con.prepareStatement(sql);
            ps.setDate(1, srenew_date);
            ps.setDate(2, sdue_date);
            ps.setString(3, fine_days);
            ps.setString(4, total_fine);
